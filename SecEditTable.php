...
<div class="container">
    <div class="title">
        <h2 style='text-align:center; background-color: #39cccc; font-weight: 700;margin-top:5%;font-family: "Classic Official", sans-serif;'>
            <span style="color: black;">All IIT Editable Fund Information</span>
    ...
    <div class="table-responsive">
        <table class="table" id="fundTable" style="border: 2px solid black; border-collapse: collapse;">
            ...
            <tbody>
                ...
                $query3 = "SELECT iit_name.*, anothertable.*
                          FROM iit_name
                          LEFT JOIN anothertable ON iit_name.name = anothertable.iit_name
                          WHERE anothertable.financial_year = '$selected_financial_year'
                          AND anothertable.curr_quarter = '$selected_quarter' ORDER BY iit_name.name ASC";
                $result3 = mysqli_query($con, $query3);
                ...
                if ($result->num_rows > 0) {
                    ...
                    // Start loop to fetch and display data
                    while ($row = $result->fetch_assoc()) {
                        $iit_name = $row['name'];
                
                        // Fetching other data here...
                
                        $netFundSought =  $Total_Amount_sought + $Total_Funds_lapsed_and_not_reallocated - $Excess_fund_last_quarter;
                
                        // Output table row with editable cells
                        echo "<tr>";
                        echo "<td>{$serialNumber}</td>"; // Display serial number
                        echo "<td contenteditable='true' class='integerInput'>{$iit_name}</td>";
                        echo "<td contenteditable='true' class='integerInput'>{$Fund_available_PFMS}</td>";
                        echo "<td contenteditable='true' class='integerInput'>{$Total_Funds_lapsed_last_quarter}</td>";
                        echo "<td contenteditable='true' class='integerInput'>{$TFR_since_2020}</td>";
                        echo "<td contenteditable='true' class='integerInput'>{$Total_Amount_sought}</td>";
                        echo "<td contenteditable='true' class='integerInput'>{$Total_Funds_lapsed_and_not_reallocated}</td>";
                        echo "<td contenteditable='true' class='integerInput'>{$Excess_fund_last_quarter}</td>";
                        echo "<td contenteditable='true' class='integerInput'>{$netFundSought}</td>";
                        echo "</tr>";
                    ...
                    echo "<tr>";
                    echo "<td colspan='5' style='background-color:yellow;'><strong>Total</strong></td>";
                    echo "<td id='totalC' style='background-color:yellow;'>{$totalC}</td>";
                    echo "<td id='totalA' style='background-color:yellow;'>{$totalA}</td>";
                    echo "<td id='totalB' style='background-color:yellow;'>{$totalB}</td>";
                    echo "<td id='totalNetFundSought' style='background-color:yellow;'>{$totalNetFundSought}</td>";
                    echo "</tr>";
                } else {
                    echo "<tr><td colspan='5'>No data found.</td></tr>";
                }
  ...
  Download Excel
        ...
        <script>
            ...
            function downloadExcel() {
                ...
                // Add event listeners to cells with the class 'integerInput' to allow only integer input
                document.querySelectorAll('.integerInput').forEach(function(cell) {
                    cell.addEventListener('input', function() {
                        if (!/^\d*$/.test(this.textContent)) {
                            // If the entered value is not an integer, revert the content to its previous value
                ...
                // Add event listener to cells with class 'integerInput' for input changes
                document.querySelectorAll('.integerInput').forEach(function(cell) {
                    cell.addEventListener('input', function() {
                        recalculateTotals(); // Recalculate totals on input change
                    });
                });
                ...
                function recalculateTotals() {
                    ...
                    // Iterate over table rows to sum up values
                    document.querySelectorAll('#fundTable tbody tr').forEach(function(row) {
                        var cells = row.querySelectorAll('td');

                        // Skip row if it's the total row
                        if (!row.classList.contains('totalRow')) {
                            var Total_Amount_sought = parseInt(cells[5].textContent);
                            var Total_Funds_lapsed_and_not_reallocated = parseInt(cells[6].textContent);
                            var Excess_fund_last_quarter = parseInt(cells[7].textContent);
                    ...
                    // Calculate total net fund sought
                    var totalNetFundSought = totalC + totalA - totalB;
                    ...
                    document.getElementById('totalNetFundSought').textContent = totalNetFundSought;
                }
